...
sir_values_1 = sir_1(beta = 0.132,gamma = 0.0909,S0 = 8310127, I0 = 11087, R0 = 8869, times = 0:365)
View(sir_values_1)
with(sir_values_1, {
...
# adding a legend:
legend(190,7900000, c("susceptibles", "infectious", "recovered"),
       col = c("blue", "red", "green"),lwd = 3, lty = 1, bty = "c", cex = 1.25)
#Comparing a model's predictions with data
dat = read.table("C:\\Users\\thoidaipc\\Documents\\statistics\\sir.txt", header = T)
View(dat)
attach(dat)
model_fit <- function(beta, gamma, dat, N = 8330083, ...) {
  ...
  predictions <- sir_1(beta = beta, gamma = gamma,   # parameters
                       S0 = 8310127, I0 = 11087, R0 = 8869, # variables' intial values
  ...
  plot(times, dat$cases, col = "red",type = "l", xlab = "days",ylab = "cases", lwd = 4
       , ylim = c(0,500000), cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
...
model_fit(beta = 0.135, gamma = 0.088, dat, pch = 19, col = "red",
          cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
